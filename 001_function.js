...
함수(a = 100, c = 300)
함수(a = 100, b = 300, c = 200) // 순서가 뒤바뀌면 hell / 함수에서 값을 뒤바꾸면서 줄수 없다
...
// 어떻게 사용하고 원리는 무엇인가요?
function 로그인정보({
    회원등급 = 'Gold', // 이것은 할당이기때문에 오브젝트가 아님
    글쓰기 = true,
    글읽기 = true,
    채널관리 = true,
    백업 = '1주일 이내 가능',
...
let { one:10, two, three } = { two: 20, one: 10, three: 30 } // one은 변수 이기 때문에 : 로 하면 error
let { one = 100, two, three } = { two: 20, three: 30 } // one은 변수 이기 때문에 = 로 해야한다
...
// 참고삼아서만 알아두세요.
// let {one = 1, two = 2} = {one:100} // 결과 -> one 100 , two 2
// let {one = 1, two = 2} = {} // 결과 -> one 1 , two 2
// let {a=10, b=20, c=30} = undefined // 결과 -> undefined
